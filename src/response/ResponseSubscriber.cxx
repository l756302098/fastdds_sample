...
ResponseSubscriber::ResponseSubscriber()
    ...
    , topic_(nullptr)
    , reader_(nullptr)
    , type_(new mind_interfaces::msg::ResponsePubSubType())
{
}
...
bool ResponseSubscriber::init(const std::string& name,std::function<void(const int64_t,const std::string&)> cb)
{
...
void ResponseSubscriber::SubListener::on_subscription_matched(
        DataReader*,
...
void ResponseSubscriber::SubListener::on_data_available(
        DataReader* reader)
{
    // Take data
    ...
    if (reader->take_next_sample(&st, &info) == ReturnCode_t::RETCODE_OK)
    {
        if (info.valid_data)
        {
            ...
            if(this->callback)
            {
                this->callback(st.index(),st.data().to_string());
            }
        }
    }
}

void ResponseSubscriber::run()
{
